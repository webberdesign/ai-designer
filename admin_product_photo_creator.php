<?php
/*
 * Admin Product Photo Creator
 *
 * This tool generates or enhances product photos. Administrators can enter
 * a product name and an optional style/mood description, choose a
 * background colour and optionally upload a reference image. If an
 * image is uploaded, the Gemini model uses it as a base for editing;
 * otherwise it generates a new photo from the description. Generated
 * images are stored in the common JSON DB with the 'photo' tool tag.
 */

// Load configuration for API keys and models
require_once __DIR__ . '/config.php';
// Define constants for models based on configuration
...
$DB_FILE    = __DIR__ . '/photo_designs.json';
...
// Determine default selected model for the form; default to OpenAI
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'generate_design') {
    ...
    // Prepare inline data only if using Gemini
    $inlineData = null;
    ...
    // Determine aspect ratio for product photo (square by default)
    ...
    // Build prompt: use editing or generation wording, including ratio description
    if ($chosenModel === 'gemini' && $inlineData) {
        $prompt = sprintf(
            'Product photo editing, %s aspect ratio. Use the uploaded image as the base. Enhance lighting and clarity, set background to solid %s. Highlight the product: "%s". Style: %s. Realistic and polished.',
            $ratioDesc,
            $bgColor,
            $product,
            $style ?: 'natural'
        ...
        // For OpenAI and Gemini when no reference image
        $prompt = sprintf(
            'High-quality product photo of "%s", %s aspect ratio. Style: %s. Use a solid %s background. Realistic, polished, professional.',
            $product,
            $ratioDesc,
            $style ?: 'natural',
            $bgColor
    ...
    if ($chosenModel === 'gemini') {
        ...
        // Use OpenAI for product photos; ignore reference images. Pass configured model name as final parameter.
        // For OpenAI generation, do not pass the hex colour as the background parameter. The colour is conveyed via the prompt. Use null background.
        // Use dynamic size for OpenAI based on ratio
        $openResult = send_openai_image_request($prompt, $background, $OPENAI_API_KEY, $size, OPENAI_MODEL);
        if (!$openResult[0]) {
    ...
    try {
        $filename = 'photo_' . date('Ymd_His') . '_' . bin2hex(random_bytes(3)) . '.' . $ext;
    } catch (Exception $e) {
        $filename = 'photo_' . date('Ymd_His') . '_' . mt_rand(1000, 9999) . '.' . $ext;
    }
...
$designsFiltered = array_filter($designsAll, function ($item) {
    return ($item['tool'] ?? '') === 'photo';
});
$designs = array_reverse($designsFiltered);
$preview = $designs[0] ?? null;
$default_product = isset($_POST['product_name']) ? trim($_POST['product_name']) : (isset($_GET['text']) ? trim($_GET['text']) : '');
...
// Determine default aspect ratio. Product photos are square by default.
...
<head>
    ...
    <title>Admin — Product Photo Creator</title>
...
<div class="admin-container">
    ...
    <div class="admin-main">
        ...
        <section class="admin-content">
            <h1>Product Photo Creator</h1>
            <div class="page-shell">
                <section class="card">
                    <div class="card-title">Product Photo Prompt</div>
                    <div class="card-sub">Enter the product name and optionally describe the desired style or mood. You can upload a reference photo to enhance; otherwise the model will generate a new image.</div>
                    <form id="design-form" method="post" enctype="multipart/form-data" class="form-grid">
                        ...
                        <div>
                            ...
                            <input type="text" id="product-name" name="product_name" class="input-text" placeholder="e.g. Ceramic Mug" required value="<?php echo htmlspecialchars($default_product); ?>">
                        </div>
                        <div>
                            <label class="field-label" for="style-text">Style &amp; Mood</label>
                            <input type="text" id="style-text" name="style_text" class="input-text" placeholder="e.g. minimalist, natural light" value="<?php echo htmlspecialchars($default_style); ?>">
                        </div>
                        <div>
                ...
                <section class="preview-shell">
                    <div class="preview-card">
                        <div class="preview-meta">
                            <div>
                                <span style="font-size:0.75rem;color:#9ca3af;">Latest product photo</span><br>
                                <strong id="latest-title"><?php echo $preview ? htmlspecialchars($preview['display_text']) : 'No product photos yet'; ?></strong>
                            </div>
                        ...
                        <div class="preview-frame" id="preview-frame">
                            <?php if ($preview): ?>
                                ...
                                <img id="preview-image" src="<?php echo 'generated_tshirts/' . rawurlencode($preview['file']); ?>" alt="Latest product photo">
                            <?php else: ?>
                                <span class="preview-badge" id="preview-badge">Waiting…</span>
                                <img id="preview-image" src="" alt="" style="display:none;">
                                <span id="preview-placeholder" style="font-size:0.8rem;color:#6b7280;padding:0 16px;text-align:center;">Your first product photo will appear here after you hit “Generate”.</span>
                            <?php endif; ?>
                        </div>
                        <div class="meta-line" id="meta-product">
                            <?php if ($preview): ?><span class="key">Product:</span> <?php echo htmlspecialchars($preview['display_text']); ?><?php endif; ?>
                        </div>
                        <div class="meta-line" id="meta-style">
                            <?php if ($preview): ?><span class="key">Style:</span> <?php echo htmlspecialchars($preview['graphic'] ?? ''); ?><?php endif; ?>
                        </div>
                    ...
                    <div class="gallery-card">
                        <div class="gallery-title-row">
                            <div class="gallery-title">Recent product photos</div>
                        ...
                        <div class="gallery-grid" id="gallery-grid">
                            <?php if (!empty($designs)): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div style="font-size:0.8rem;color:#6b7280;">Product photos you generate will show up here as a mini gallery.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
...
(function() {
    ...
    function stopLoading() {
        ...
        if (submitBtn) {
            ...
            submitBtn.querySelector('.icon').textContent = '⚡';
            submitBtn.querySelector('span:last-child').textContent = 'Generate Product Photo';
        }
    }
    ...
    function updatePreview(design) {
        if (!design) return;
        if (previewTitle) previewTitle.textContent = design.display_text || 'Latest product photo';
        if (previewImg) {
        ...
        if (metaProduct) metaProduct.innerHTML  = '<span class="key">Product:</span> ' + escapeHtml(design.display_text || '');
        if (metaStyle)   metaStyle.innerHTML    = '<span class="key">Style:</span> ' + escapeHtml(design.graphic || '');
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            const prod = (fd.get('product_name') || '').toString().trim();
            if (!prod) {
