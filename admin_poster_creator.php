 ...
 *
 * This page allows administrators to generate custom poster designs using
 * Google's Gemini image model. Posters are stored alongside other designs
 * in the main JSON database but are tagged with the 'poster' tool so
 * they can be filtered in the gallery. A reference image may be uploaded
 * to influence the generation – if provided the image is sent to the
 * model as inline data. The layout mirrors the existing admin design lab
 * pages, with a form on the left and a live preview/gallery on the right.
 */

// Model and API configuration loaded from the central config file
require_once __DIR__ . '/config.php';
// Define constants for models from configuration if not already defined
...
$DB_FILE    = __DIR__ . '/poster_designs.json';
...
// Handle AJAX generation for posters
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'generate_design') {
    ...
    // Determine if a transparent background was selected. Only use 'transparent' or null for the OpenAI API.
    $transparent = isset($_POST['transparent_bg']);
    $background  = $transparent ? 'transparent' : null;
    // Require at least a title and graphic description
    if ($title === '' || $graphic === '') {
        echo json_encode(['success' => false, 'error' => 'Please enter both the poster title and the imagery description.']);
        exit;
    }
    // Choose model; default to Gemini
    $chosenModel = $_POST['image_model'] ?? 'gemini';
    // Determine aspect ratio for poster generation
    ...
    // Build prompt for poster design including aspect ratio
    $prompt = sprintf(
        'Poster design, %s. Use a solid %s background. Title: "%s". Subtitle: "%s". Incorporate imagery: %s. ' .
        'Bold typography, modern layout, balanced composition, high contrast.',
        $ratioDesc,
        $bgColor,
        $title,
        $subtitle,
    ...
    // Handle reference image (only used for Gemini)
    $inlineData = null;
    ...
    if ($chosenModel === 'openai') {
        ...
        // Call OpenAI with the configured model name using the size based on aspect ratio
    ...
    try {
        $filename = 'poster_' . date('Ymd_His') . '_' . bin2hex(random_bytes(3)) . '.' . $ext;
    } catch (Exception $e) {
        $filename = 'poster_' . date('Ymd_His') . '_' . mt_rand(1000, 9999) . '.' . $ext;
    }
    ...
    // Save record to DB with source and tool
...
// Preserve the selected image model between submissions; default to Gemini
$default_model = isset($_POST['image_model']) ? $_POST['image_model'] : 'gemini';

// Determine default aspect ratio. Preserve selection across submissions; default to portrait (2:3).
...
<head>
    ...
    <title>Admin — Poster Creator</title>
    <!-- Google Font -->
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Dropdown select styled like inputs */
        .input-select {
            width: 100%;
            padding: 10px 11px;
            border-radius: 8px;
...
<div class="admin-container">
    ...
    <div class="admin-main">
        <?php
            // Determine active page for nav highlighting. Group all creator pages under 'Create'.
        ...
        <section class="admin-content">
            ...
            <div class="page-shell">
                <!-- Form Card -->
                <section class="card">
                    <div class="card-title">Poster Prompt</div>
                    <div class="card-sub">Enter the poster’s title, optional subtitle, and imagery description. You can also upload a reference image to inspire the design.</div>
                    <form id="design-form" method="post" enctype="multipart/form-data" class="form-grid">
                        ...
                        <div>
                            <label class="field-label" for="title-text">Title <span class="required">*</span></label>
                            <input type="text" id="title-text" name="title_text" class="input-text" placeholder="e.g. Spring Festival" required value="<?php echo htmlspecialchars($default_title); ?>">
                        </div>
                        <div>
                            <label class="field-label" for="subtitle-text">Subtitle / Tagline</label>
                            <input type="text" id="subtitle-text" name="subtitle_text" class="input-text" placeholder="e.g. Celebrate the season" value="<?php echo htmlspecialchars($default_subtitle); ?>">
                        </div>
                        <div>
                            <label class="field-label" for="graphic-prompt">Imagery Description <span class="required">*</span></label>
                            <textarea id="graphic-prompt" name="graphic_prompt" class="input-text" rows="3" placeholder="e.g. blooming flowers, colourful confetti" required><?php echo htmlspecialchars($default_graphic); ?></textarea>
                        </div>
                        <div>
                ...
                <section class="preview-shell">
                    <div class="preview-card">
                        <div class="preview-meta">
                            <div>
                                <span style="font-size:0.75rem;color:#9ca3af;">Latest poster</span><br>
                                <strong id="latest-title"><?php echo $preview ? htmlspecialchars($preview['display_text']) : 'No posters yet'; ?></strong>
                            </div>
                        ...
                        <div class="preview-frame" id="preview-frame">
                            <?php if ($preview): ?>
                                ...
                                <img id="preview-image" src="<?php echo 'generated_tshirts/' . rawurlencode($preview['file']); ?>" alt="Latest poster">
                            <?php else: ?>
                                <span class="preview-badge" id="preview-badge">Waiting…</span>
                                <img id="preview-image" src="" alt="" style="display:none;">
                                <span id="preview-placeholder" style="font-size:0.8rem;color:#6b7280;padding:0 16px;text-align:center;">Your first poster will appear here after you hit “Generate”.</span>
                            <?php endif; ?>
                        </div>
                        <div class="meta-line" id="meta-title">
                            <?php if ($preview): ?><span class="key">Title:</span> <?php echo htmlspecialchars($preview['display_text']); ?><?php endif; ?>
                        </div>
                        <div class="meta-line" id="meta-subtitle">
                            <?php if ($preview): ?><span class="key">Subtitle:</span> <?php echo htmlspecialchars($preview['subtitle'] ?? ''); ?><?php endif; ?>
                        </div>
                    ...
                    <div class="gallery-card">
                        <div class="gallery-title-row">
                            <div class="gallery-title">Recent posters</div>
                        ...
                        <div class="gallery-grid" id="gallery-grid">
                            <?php if (!empty($designs)): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div style="font-size:0.8rem;color:#6b7280;">Posters you generate will show up here as a mini gallery.</div>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
...
(function() {
    ...
    function stopLoading() {
        ...
        if (submitBtn) {
            ...
            submitBtn.querySelector('.icon').textContent = '⚡';
            submitBtn.querySelector('span:last-child').textContent = 'Generate Poster';
        }
    }
    ...
    function updatePreview(design) {
        if (!design) return;
        if (previewTitle) previewTitle.textContent = design.display_text || 'Latest poster';
        if (previewImg) {
        ...
        if (metaTitle) metaTitle.innerHTML    = '<span class="key">Title:</span> ' + escapeHtml(design.display_text || '');
        if (metaSubtitle) metaSubtitle.innerHTML = '<span class="key">Subtitle:</span> ' + escapeHtml(design.subtitle || '');
    ...
    // Drag & drop reference image handling
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (!title || !graphic) {
                setError('Please enter both the poster title and the imagery description.');
                return;
            }
            startLoading();
